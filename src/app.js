...
class Superwar {
  ...
  calculateScore = () => {
    ...
    // Type your code here
    let score = this.players.reduce((totalScore, player) => {
      return {
        hero: totalScore.hero + (player.type == 'hero' ? player.wins : 0),
        villain: totalScore.villain + (player.type == 'villain' ? player.wins : 0),
      };
    }, { hero: 0, villain: 0 });
  ...
  checkWin = () => {
    ...
    result =
      totalStrengthOfHero == totalStrengthOfVillain
        ? 'endure'
        : totalStrengthOfHero > totalStrengthOfVillain
        ? 'hero'
        : 'villain';
